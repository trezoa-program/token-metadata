...
to be considered a "token-metadata" program for TPL token mints. The interface
...
Token creators on Trezoa need all sorts of functionality for their token-metadata,
and the Trezoaplex Token-Metadata program has been the one place for all metadata
...
[tpl-type-length-value repo](https://github.com/trzledgerfoundation/trezoa-program-library/tree/master/libraries/type-length-value).
